...
/**
 * Output port — fetches all products for a given level-1 subcategory and maps them to {@link
 * UpsertProductCommand} objects ready for the application layer.
 */
public interface ProductScraperPort {

  /**
   * Calls GET /api/categories/{level1ExternalId} and returns one {@link UpsertProductCommand} per
   * product found across all leaf groups.
   *
   * @param supermarketId the supermarket this sync belongs to
   * @param level1ExternalId Mercadona's own id for the level-1 subcategory
