...
- Add `gradle.properties` with JVM args needed by google-java-format on Java 25+
...
- **Note:** Spring Boot 4 removed `@WebMvcTest`; tests use `MockMvcBuilders.standaloneSetup()` instead
- **Verify:** standalone MockMvc test verifying 404/422/409/403/502 → `ProblemDetail` with correct status
...
- `micrometer-registry-otlp`, `micrometer-tracing-bridge-otel`, `logstash-logback-encoder` already in `libs.versions.toml`
- Actuator endpoints configured in `application.yaml`
- Add Logback JSON config (`logback-spring.xml`) — JSON in prod/staging, human-readable in local/test
...
- Create `README.md` with quick-start, code quality table and hook install instructions
